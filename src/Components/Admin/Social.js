...
import Duzenle from './Social/Duzenle';
import Sil from './Social/Sil';
import YeniEkle from './Social/YeniEkle';

const Social = () => {
    const [isLoaded, setIsLoaded] = useState(false);
    const [events, setEvents] = useState([]);
    useEffect(() => {
        axios.get(window.apiBase + '/socials').then((response, err) => {
            if (err) {
                console.log(err);
            } else {
                ...
                isLoaded ?
                    <div>
                        <div className='max-w-7xl mx-auto'>
                            ...
                            <div>
                                <p className='text-5xl uppercase font-bold font-sans text-center border-b mb-3 text-firstColor'>Sosyal</p>
                                <div className='grid md:grid-cols-3'>
                                    {
                                        events.map(item => (
                                            <div key={uuid()} className="bg-white shadow-md border border-gray-200 rounded-lg m-1">
                                                <Link to={'/sosyal/' + item.href}><img className="rounded-t-lg" src={item.src} alt="" /></Link>
                                                <div className="p-5">
