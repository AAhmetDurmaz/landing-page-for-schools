...
import Duzenle from './Blog/Duzenle.js';
import Sil from './Blog/Sil';
import YeniEkle from './Blog/YeniEkle';

const Blog = () => {
    const [isLoaded, setIsLoaded] = useState(false);
    const [posts, setPosts] = useState([]);
    useEffect(() => {
        axios.get(window.apiBase + '/blog').then((response, err) => {
            if (err) {
                console.log(err);
            } else {
                ...
                isLoaded ?
                    <div>
                        <div className='max-w-7xl mx-auto'>
                            ...
                            <div>
                                <p className='text-5xl uppercase font-bold font-sans text-center border-b mb-3 text-firstColor'>Öğrencilerimizden Yazılar/Blog</p>
