...
const Sil = (props) => {
    ...
    const removeUser = async () => {
        await axios.post((window.apiBase + '/remove-custom-text'), { id: props.id }, { headers: { 'Authorization': 'Bearer ' + cookies.get('auth') } }
        ).then((response, err) => {
            if (err) {
                console.log(err);
    ...
    return (
        <div>
            ...
            <button onClick={() => { setOpen(true); }} className='bg-red-600 hover:bg-red-800 text-white px-5 py-2 rounded-lg transition font-sans font-bold'>Yazıyı Sil</button>
        </div>
    )
}
